...
class ME(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # self.depthwise_conv_reduce_channels_1 = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=1)

        self.depthwise_conv_reduce_channels_1 = torch.nn.Conv2d(in_channels, out_channels, kernel_size=1,
    ...
    def forward(self, dense_embeddings_boundary, dense_embeddings_box, high_frequency, sparse_embeddings_box):  # high_frequency, 这里删了一个这个

        dense_cat = torch.cat([dense_embeddings_boundary, dense_embeddings_box], dim=1)
        dense_cat_tmp = self.depthwise_conv_reduce_channels_1(dense_cat)
